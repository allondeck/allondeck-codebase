    ...
    if (url.startsWith("http")) {
      parts.push(
        <a
          ...
          className="underline underline-offset-2 hover:text-brand-orange font-semibold text-brand-cream"
        >
          {label}
        </a>,
      );
    ...
    <>
      <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
        ...
        <button
          ...
          className="flex h-14 w-14 items-center justify-center rounded-full bg-brand-orange text-white shadow-xl transition-all duration-200 hover:scale-105 hover:bg-orange-600 focus:outline-none focus:ring-2 focus:ring-brand-orange focus:ring-offset-2"
        >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-24 right-6 z-50 flex w-[350px] flex-col overflow-hidden rounded-2xl border border-brand-medium/35 bg-brand-dark text-white shadow-2xl"
            style={{ height: "500px" }}
            role="dialog"
            aria-label="AI Chat Assistant"
          >
            {/* Header */}
            <div className="flex items-center gap-3 border-b border-brand-medium/35 bg-brand-dark px-4 py-3">
              <div className="flex h-8 w-8 items-center justify-center rounded-full bg-brand-medium/40 text-brand-cream">
                <svg
                  className="h-4 w-4"
                  fill="none"
                  stroke="currentColor"
              ...
              <div className="ml-auto flex items-center gap-3">
                ...
                <button
                  ...
                  className="flex h-6 w-6 items-center justify-center rounded-md text-brand-light/60 transition-colors hover:bg-brand-medium/30 hover:text-white"
                >
                  <svg
                    className="h-4 w-4"
                    fill="none"
              ...
              {messages.length === 0 ? (
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex gap-2">
                    <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-brand-medium text-white">
                      <svg
                        className="h-4 w-4"
                        fill="none"
                        stroke="currentColor"
                  ...
                  <div className="flex flex-wrap gap-2 pl-9">
                    {SUGGESTED_PROMPTS.map((prompt) => (
                      <button
                        ...
                        disabled={isLoading}
                        className="rounded-full border border-brand-light/40 px-3 py-1 text-xs text-brand-light transition-colors hover:border-brand-orange hover:bg-brand-orange hover:text-white disabled:opacity-50 font-sans"
                      >
                        {prompt}
                      </button>
                    ))}
            ...
            <form
              ...
              className="flex items-center gap-2 border-t border-brand-medium/35 bg-[#032e3b] px-3 py-3"
            >
              <input
                type="text"
                value={input}
                ...
                disabled={isLoading}
                className="flex-1 rounded-full border border-brand-medium/50 bg-brand-dark text-white px-4 py-2 text-sm placeholder-brand-light/50 focus:border-brand-orange focus:outline-none focus:ring-1 focus:ring-brand-orange disabled:opacity-60 font-sans"
              />
              <button
                type="submit"
                disabled={!input.trim() || isLoading}
                aria-label="Send message"
                className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-brand-orange text-white transition-all hover:bg-orange-600 disabled:opacity-40"
              >
                <svg
                  className="h-4 w-4"
                  fill="none"
