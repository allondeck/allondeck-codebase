...
const navItems = [
  ...
  { to: "/account/owner/services", label: "Services" },
...
export function OwnerLayout() {
  ...
  return (
    <div className="mx-auto max-w-content px-6 lg:px-12 pb-20 pt-8">
      <div className="flex flex-col gap-6 md:flex-row md:gap-8">
        {/* Vertical sidebar - desktop */}
        <aside className="hidden w-48 shrink-0 md:block text-left">
          <div className="sticky top-24 space-y-1 rounded-lg border border-brand-medium/35 bg-brand-dark-alt p-2">
              ...
              return (
                <Link
                  ...
                  className={`block rounded-md px-3 py-2 text-sm font-semibold transition-all ${
                    isActive
                      ? "bg-brand-medium text-brand-cream shadow-md border-l-4 border-brand-orange"
                      : "text-brand-cream/80 hover:bg-brand-medium/30 hover:text-white"
                  }`}
                >
                ...
                <Link
                  ...
                  className={`shrink-0 rounded-full px-4 py-1.5 text-sm font-semibold transition ${
                    isActive
                      ...
                      : "bg-brand-dark-alt text-brand-cream/80 border border-brand-medium/30 hover:bg-brand-medium/30 hover:text-white"
                  }`}
                >
                  {label}
                </Link>
