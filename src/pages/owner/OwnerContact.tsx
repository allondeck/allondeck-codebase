...
export default function OwnerContact() {
  ...
  return (
    <div>
      <h2 className="text-xl font-semibold text-brand-cream">Contact & Estimates</h2>
      ...
      {requests.length === 0 ? (
        <div className="mt-8 rounded-xl border-2 border-dashed border-brand-medium/35 bg-brand-dark-alt p-12 text-center">
          <p className="text-brand-light">No messages yet.</p>
        </div>
      ) : (
        <div className="mt-6 space-y-3">
          {requests.map((r) => {
            ...
            return (
              <div
                key={r.id}
                className="rounded-xl border border-brand-medium/35 bg-brand-dark-alt shadow-sm overflow-hidden"
              >
                <button
                  type="button"
                  onClick={() => setExpandedId(expandedId === r.id ? null : r.id)}
                  className="flex w-full flex-wrap items-center justify-between gap-3 p-4 text-left hover:bg-brand-medium/20"
                >
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="font-medium text-white">{r.name}</p>
                      {isEstimate && (
                        <span className="rounded-full bg-brand-orange/20 border border-brand-orange/40 px-2 py-0.5 text-[10px] font-bold text-brand-orange tracking-wider">
                          ESTIMATE
                        </span>
                      )}
                      {r.subject && !isEstimate && (
                        <span className="rounded-full bg-brand-medium/40 px-2 py-0.5 text-[10px] font-medium text-brand-light">
                          {r.subject}
                        </span>
                      )}
                    </div>
                    <p className="text-sm text-brand-light">{r.email}</p>
                    {(r as any).phone && (
                      <p className="text-xs text-brand-light/70">{(r as any).phone}</p>
                    )}
                    {expandedId !== r.id && (
                      <p className="mt-1 line-clamp-1 text-sm text-brand-light">
                        {r.message}
                      </p>
                    )}
                  </div>
                ...
                {expandedId === r.id && (
                  <div className="border-t border-brand-medium/35 bg-brand-medium/10 p-4 space-y-3">
                    {/* Boat / estimate details */}
                    {isEstimate && (
                      <div className="grid grid-cols-2 gap-2 text-xs rounded-lg bg-[#033343] p-3">
                        {(r as any).phone && (
                          <div>
                            <span className="text-brand-light uppercase tracking-wider font-bold">Phone</span>
                        ...
                        {(r as any).notes && (
                          <div className="col-span-2">
                            <span className="text-brand-light uppercase tracking-wider font-bold">Details</span>
                    ...
                    <div>
                      <p className="text-xs font-bold uppercase tracking-wider text-brand-light mb-1">Message</p>
