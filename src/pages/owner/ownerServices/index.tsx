  ...
  function openServiceForm(item?: ServiceRow) {
    if (item) {
      ...
      setServiceFormData({
        ...
        cta_link: item.cta_link || "/estimate",
  ...
  async function handleSaveService(e: React.FormEvent) {
    ...
    try {
      const payload = {
        ...
        cta_text: serviceFormData.cta_text.trim() || "Get Started",
        cta_link: serviceFormData.cta_link.trim() || "/estimate",
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <p className="text-xs sm:text-sm text-brand-light">
                Each service is rendered dynamically on the public Services page in alternating dark & medium themed sections.
              </p>
            </div>
            <button
          ...
          {isServiceFormOpen && (
            <div className="rounded-xl border border-brand-medium/40 bg-brand-dark-alt p-6 shadow-xl animate-in fade-in">
              <h3 className="mb-4 text-lg font-bold text-brand-cream font-heading border-b border-brand-medium/30 pb-3">
                {editingServiceId ? "Edit Service Section" : "Create New Service Section"}
              </h3>
              <form onSubmit={handleSaveService} className="space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    required
                    rows={3}
                    placeholder="Comprehensive explanation of what this marine service includes..."
                ...
                <div>
                  <label className="block text-xs font-semibold uppercase text-brand-light mb-1">
                    Secondary / Supporting Description (Optional)
                  </label>
                  <textarea
                    rows={2}
                    placeholder="Additional details, italic note, or secondary bullet points..."
                ...
                <div>
                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                    {serviceFormData.image_url ? (
                      ...
                      <div className="flex h-24 w-36 items-center justify-center rounded-lg border border-dashed border-brand-medium/50 bg-brand-dark text-brand-light">
                        <ImageIcon className="h-8 w-8" />
                      </div>
                    )}

                    <div className="flex flex-col gap-2 flex-1">
                      <label className="inline-flex cursor-pointer items-center gap-2 rounded-md border border-brand-orange/50 bg-brand-orange/20 px-4 py-2 text-xs font-semibold text-brand-orange hover:bg-brand-orange/30 transition-colors w-fit">
                        {uploadingServiceImg ? (
                          <>
                      ...
                      <input
                        type="text"
                        placeholder="Or enter image URL path e.g. /assets/images/1.jpg"
                        value={serviceFormData.image_url}
                        onChange={(e) =>
                          setServiceFormData({ ...serviceFormData, image_url: e.target.value })
                        }
                        className="w-full rounded-md border border-brand-medium/50 bg-brand-dark px-3 py-1.5 text-xs text-white"
                      />
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 pt-2">
                  ...
                  />
                  <label htmlFor="service_is_active" className="text-sm font-medium text-brand-light cursor-pointer">
              ...
              <table className="w-full min-w-[750px] divide-y divide-brand-medium/35 text-left text-sm">
                ...
                <tbody className="divide-y divide-brand-medium/35">
                  {services.map((srv, idx) => (
                    <tr key={srv.id} className="hover:bg-brand-medium/20 transition-colors">
                      <td className="whitespace-nowrap px-4 py-3 text-brand-light">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex items-center gap-1.5">
                            <button
                              disabled={idx === 0}
                              onClick={() => handleReorderService(srv.id, -1)}
                              title="Move Up"
                      ...
                      <td className="whitespace-nowrap px-4 py-3">
                        <span className="inline-block rounded-md bg-brand-medium/40 border border-brand-medium/50 px-2 py-1 text-xs text-brand-light">
                          {srv.cta_text || "Get Started"} → {srv.cta_link || "/estimate"}
                        </span>
                      </td>
                      <td className="whitespace-nowrap px-4 py-3">
                        <button
        ...
        <div className="space-y-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <p className="text-xs sm:text-sm text-brand-light">
                Upload and categorize custom boat decking and manufacturing photos for the project gallery.
              </p>
            </div>
            <button
          ...
          {isGalleryFormOpen && (
            <div className="rounded-xl border border-brand-medium/40 bg-brand-dark-alt p-6 shadow-xl animate-in fade-in">
              <h3 className="mb-4 text-lg font-bold text-brand-cream font-heading border-b border-brand-medium/30 pb-3">
                {editingGalleryId ? "Edit Gallery Image" : "Upload New Gallery Image"}
              </h3>
              <form onSubmit={handleSaveGallery} className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setGalleryFormData({ ...galleryFormData, service_type: e.target.value })
                      }
                      className="w-full rounded-md border border-brand-medium/50 bg-brand-dark px-3 py-2 text-sm text-white shadow-sm focus:border-brand-orange focus:outline-none focus:ring-1 focus:ring-brand-orange"
                    >
                      {SERVICE_TYPE_OPTIONS.map((opt) => (
                ...
                <div className="flex justify-end gap-3 pt-4 border-t border-brand-medium/30">
                  ...
                  >
                    ...
                    {savingGallery ? "Saving..." : "Save Image"}
                  </button>
                </div>
              </form>
            </div>
                      ...
                      <td className="whitespace-nowrap px-4 py-3 text-brand-light">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex items-center gap-1.5">
                            <button
                              disabled={idx === 0}
                              onClick={() => handleReorderGallery(img.id, -1)}
                              title="Move Up"
