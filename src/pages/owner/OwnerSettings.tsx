 ...
 * OwnerSettings.tsx
 * 
 * This is the main Settings page for the store owner.
 * It manages global store configuration including:
 * - Store identity (Logo, Name)
 * - Shipping configurations (Estimated delivery, supported countries)
...
import { OwnerSettingsShipping } from "./components/OwnerSettingsShipping";
 ...
 * and performing the final database upsert on save.
 */
export default function OwnerSettings() {
  ...
  const sectionCard = "rounded-xl border border-brand-medium/35 bg-brand-dark-alt shadow-sm";
  const sectionHeader =
        ...
        <section className={sectionCard}>
          <div className="p-4 sm:p-6">
            ...
            <div className="space-y-6">
              <div className="rounded-lg border border-brand-medium/35 bg-brand-medium/15 p-4">
                ...
                <label className="mt-4 flex cursor-pointer items-start gap-3 rounded-lg border border-brand-medium/35 bg-brand-dark-alt p-3">
                  <input
                    type="checkbox"
                    checked={showOutOfStock}
                    onChange={(e) => setShowOutOfStock(e.target.checked)}
                    className="mt-0.5 h-4 w-4 rounded border-brand-medium/60 bg-brand-dark text-brand-orange focus:ring-brand-orange"
                  />
                ...
                <div className="mt-4">
                  ...
                  <input
                    ...
                    className="mt-1 w-24 min-h-[44px] rounded-lg bg-brand-dark border border-brand-medium/60 text-white px-3 py-2 text-sm focus:border-brand-orange focus:outline-none"
                  />
                ...
                <div className="mt-4">
                  ...
                  <label className="mt-2 flex cursor-pointer items-center gap-2">
                    <input
                      ...
                      className="h-4 w-4 rounded border-brand-medium/60 bg-brand-dark text-brand-orange"
                    />
                    <span className="text-sm text-white">
          ...
          <div className="p-4 sm:p-6">
            ...
            <div>
              ...
              <textarea
                ...
                rows={5}
                className="mt-1.5 w-full rounded-lg bg-brand-dark border border-brand-medium/60 px-3 py-2.5 text-sm text-white placeholder-brand-light focus:border-brand-orange focus:outline-none focus:ring-1 focus:ring-brand-orange"
              />
          ...
          <div className="p-4 sm:p-6">
            ...
            <div className="mb-4">
              <Link
                ...
                className="inline-flex min-h-[44px] items-center justify-center rounded-lg border border-brand-medium/35 bg-brand-dark-alt px-3 py-2.5 text-sm font-medium text-white hover:bg-brand-medium/30"
              >
                See all metrics
              </Link>
            </div>
            {/* Single list: order = display order; toggle = on dashboard or not */}
            <div className="mb-4 flex flex-wrap items-center gap-2 text-sm text-brand-light">
                ...
                return combinedList.map((widgetId) => {
                  ...
                  return (
                    <li
                      ...
                      className={`flex flex-wrap select-none items-center gap-2 sm:gap-3 rounded-lg border-2 bg-brand-dark-alt px-3 py-2.5 sm:py-2 text-sm transition min-h-[48px] ${
                        draggedWidgetId === widgetId ? "opacity-60" : ""
                      } ${
                        dragOverWidgetId === widgetId
                          ? "border-brand-orange ring-2 ring-brand-orange/20"
                          : enabled
                            ? "border-brand-medium/60 bg-brand-medium/25"
                            : "border-brand-medium/20 hover:border-brand-medium/60"
                      }`}
                    >
                      <span
                        className="flex shrink-0 cursor-grab touch-none text-brand-light hover:text-white"
                      ...
                      {categoryLabel && (
                        <span className="shrink-0 rounded bg-brand-medium/30 px-2 py-0.5 text-xs text-brand-light">
                          {categoryLabel}
                        </span>
                      )}
                      <label className="relative inline-flex shrink-0 cursor-pointer items-center">
                        ...
                        />
                        <span
                          className={`relative h-6 w-11 shrink-0 rounded-full transition-colors after:absolute after:left-0.5 after:top-0.5 after:h-5 after:w-5 after:rounded-full after:bg-white after:shadow after:transition-all after:content-[''] peer-checked:bg-brand-orange peer-checked:after:translate-x-5 peer-focus-visible:ring-2 peer-focus-visible:ring-brand-orange peer-disabled:cursor-not-allowed peer-disabled:opacity-50 ${
                            enabled ? "bg-brand-orange" : "bg-brand-dark border border-brand-medium/60"
                          }`}
                          title={
                            cannotEnable
                              ? `Max ${cap} ${
        ...
        <section className={sectionCard}>
          <div className="p-4 sm:p-6">
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:gap-4">
              ...
              {cleanupResult && (
                <p className="text-sm text-brand-light">{cleanupResult}</p>
              )}
            </div>
          </div>
        </section>

        <div className="flex flex-col-reverse gap-4 border-t border-brand-medium/35 pt-6 sm:flex-row sm:flex-wrap sm:items-center">
          <Button
            type="submit"
            disabled={saving}
          ...
          <p className="text-sm text-brand-light sm:max-w-sm">
