    ...
    <div className="mx-auto max-w-md px-4 py-12 sm:px-6 text-left">
      ...
      <form onSubmit={handleSubmit} className="mt-4 space-y-4">
        ...
        <div>
          ...
          <ul
            className="mt-2 space-y-1 text-xs text-[#76abbf]"
            aria-live="polite"
          >
            {PASSWORD_REQUIREMENTS.map(({ key, label }) => (
              <li
                key={key}
                className={requirementMet(key) ? "text-emerald-400 font-medium" : "text-[#76abbf]/60"}
              >
                {requirementMet(key) ? "✓ " : "○ "}
                {label}
              </li>
      ...
      <p className="mt-4 text-center text-sm text-[#76abbf]">
        ...
        <Link to="/login" className="font-medium text-[#e38622] hover:underline">
          Sign in
        </Link>
      </p>
    </div>
