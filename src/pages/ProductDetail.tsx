...
export default function ProductDetail() {
  ...
  if (loading) {
    return (
      <div className="animate-pulse">
        <div className="aspect-square w-full max-w-lg rounded-lg bg-[#052631]" />
        <div className="mt-6 h-8 w-2/3 rounded bg-[#052631]" />
        <div className="mt-4 h-4 w-1/2 rounded bg-[#052631]" />
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-[1400px] px-6 lg:px-12 py-8 sm:px-6 lg:px-8 w-full">
      <div className="flex flex-col gap-8 lg:flex-row">
        <div className="aspect-square w-full max-w-lg shrink-0 overflow-hidden rounded-lg bg-[#052631]">
          {selectedVariant?.image_url || product.image_url ? (
            ...
            <div className="flex h-full w-full items-center justify-center text-[#76abbf]">
              <svg
                className="h-32 w-32"
                fill="none"
                stroke="currentColor"
        ...
        <div className="flex-1">
          ...
          <div className="mt-2 flex items-center gap-3">
            <span className="text-2xl font-semibold text-[#f6ebd4]">
              {formatPrice(productPrice)}
            </span>
            {hasComparePrice && (
              <span className="text-lg text-[#76abbf] line-through">
                {formatPrice(compareAtPrice)}
              </span>
            )}
          </div>
          {product.description && (
            <p className="mt-4 text-[#f6ebd4]">{product.description}</p>
          )}
          ...
          {inStock && (
            <div className="mt-6 flex flex-wrap items-center gap-4">
              <div className="flex items-center rounded-lg border border-[#066175]/50 bg-[#052631]">
                <button
                  type="button"
      ...
      {dealContainingProduct && (
        <section className="mt-12 border-t border-[#066175]/35 pt-10">
          <h2 className="mb-3 text-xl font-semibold text-white">Deal</h2>
          <p className="mb-4 text-sm text-[#f6ebd4]">
          ...
          {dealProductsLoading ? (
            <div className="flex gap-4 overflow-hidden">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="h-32 w-28 shrink-0 animate-pulse rounded-lg bg-[#052631]"
                />
              ))}
            </div>
          ) : (
            <>
                ...
                {dealContainingProduct.deal_items.map((item) => {
                  ...
                  return (
                    <Link
                      key={item.id}
                      to={`/products/${p.slug}`}
                      className="flex shrink-0 flex-col items-center gap-1 rounded-lg border border-[#066175]/35 bg-[#052631] p-3 transition hover:border-[#066175]/60 hover:bg-[#066175]/30"
                    >
                      <div className="h-20 w-20 overflow-hidden rounded-md bg-[#044155]">
                        {p.image_url ? (
                          <img
              ...
              <div className="mt-4 flex flex-wrap items-center gap-3">
                ...
                <span className="text-lg font-semibold text-[#f6ebd4]">
      ...
      {(suggested.length > 0 || suggestedLoading) && (
        <section className="mt-16 border-t border-[#066175]/35 pt-12">
      ...
      <section className="mt-16 border-t border-[#066175]/35 pt-12">
        ...
        {user &&
          ...
          canSubmitReview && (
            <form
              ...
              className="mb-8 rounded-lg border border-[#066175]/35 bg-[#052631] p-4"
            >
              <label className="block text-sm font-medium text-[#f6ebd4]">
                Your rating
              </label>
              <div className="mt-1 flex gap-1">
                {[1, 2, 3, 4, 5].map((r) => (
                  <button
                    ...
                    className={`text-2xl ${
                      reviewRating >= r ? "text-amber-500" : "text-[#066175]/50"
                    }`}
                    aria-label={`${r} stars`}
                  >
                    ★
              ...
              <label
                htmlFor="review-body"
                className="mt-3 block text-sm font-medium text-[#f6ebd4]"
              >
              ...
              <textarea
                ...
                rows={3}
                className="mt-1 w-full rounded-lg border border-[#066175]/50 bg-[#044155] px-3 py-2 text-sm text-white placeholder-[#76abbf]/50 focus:border-[#e38622] focus:outline-none focus:ring-1 focus:ring-[#e38622]"
        ...
        {reviewsLoading ? (
          <p className="text-sm text-[#76abbf]">Loading reviews...</p>
        ) : reviewsError ? (
          <p className="text-sm text-red-400">Failed to load reviews.</p>
        ) : reviews.length === 0 ? (
          <p className="text-sm text-[#76abbf]">No reviews yet.</p>
        ) : (
          <>
            <ul className="space-y-4">
              {reviews.map((r) => (
                <li
                  key={r.id}
                  className="rounded-lg border border-[#066175]/35 bg-[#052631] p-4"
                >
                  ...
                  {r.body && (
                    <p className="mt-2 text-sm text-[#f6ebd4]">{r.body}</p>
                  )}
                </li>
              ))}
            </ul>
            {hasMoreReviews && (
              <div className="mt-4">
                <button
                  ...
                  className="text-sm font-medium text-[#76abbf] underline hover:text-white disabled:opacity-50"
                >
                  {reviewsLoadingMore
                    ? "Loading..."
                    : `Show more (${Math.min(
