...
export default function Account() {
  ...
  return (
    <div className="mx-auto max-w-[1400px] px-6 lg:px-12 py-8 sm:px-6 lg:px-8 w-full text-left">
      ...
      <p className="mt-1 text-[#f6ebd4]">{profile?.full_name || user.email}</p>

      <div className="mt-8 space-y-6">
        <section className="rounded-lg border border-[#066175]/35 bg-[#052631] p-6">
          <h2 className="font-semibold text-white">Profile</h2>
          <p className="mt-1 text-sm text-[#f6ebd4]">{user.email}</p>
        ...
        {isOwner && (
          <section className="rounded-lg border border-[#e38622]/30 bg-[#e38622]/10 p-6">
            <h2 className="font-semibold text-[#e38622]">Store owner</h2>
            <p className="mt-1 text-sm text-[#f6ebd4]">
            ...
            <Link
              to="/account/owner"
              className="mt-4 inline-block rounded-lg bg-[#e38622] px-4 py-2 font-medium text-white hover:bg-orange-600 transition-all duration-200 hover:scale-105 active:scale-95"
            >
        ...
        <section className="rounded-lg border border-[#066175]/35 bg-[#052631] p-6">
          ...
          {ordersLoading ? (
            <p className="mt-2 text-sm text-[#76abbf]">Loading...</p>
          ) : orders.length === 0 ? (
            <p className="mt-2 text-sm text-[#76abbf]">No orders yet.</p>
          ) : (
            <>
              <ul className="mt-4 space-y-3">
                {orders.slice(0, 6).map((order) => (
                  <li
                    key={order.id}
                    className="flex items-center gap-4 rounded border border-[#066175]/35 bg-[#044155] p-3"
                  >
                    <div className="h-14 w-14 shrink-0 overflow-hidden rounded-md bg-[#052631]">
              ...
              {orders.length > 6 && (
                <Link
                  to="/account/orders"
                  className="mt-4 block w-full rounded-lg border border-[#066175]/50 bg-[#052631] py-2.5 text-center text-sm font-medium text-[#f6ebd4] hover:bg-[#066175]/30 transition-all duration-200 hover:scale-105 active:scale-95"
                >
        ...
        <section className="rounded-lg border border-red-950/60 bg-red-950/20 p-6">
          ...
          <p className="mt-1 text-sm text-[#f6ebd4]">
            ...
            be undone.
            {isOwner && (
              <span className="mt-2 block font-medium text-[#e38622]">
