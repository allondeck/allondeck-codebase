...
export default function OrderDetail() {
  ...
  if (error || !order) {
    return (
      <div className="rounded-xl border border-red-900/50 bg-brand-dark-alt p-8 text-center">
        ...
        <Link
          to="/account/orders"
          className="mt-6 inline-flex items-center gap-2 font-medium text-brand-light hover:text-white"
        >
    ...
    <div className="mx-auto max-w-4xl space-y-8 pb-12 text-left">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <Link
            to="/account/orders"
            className="mb-2 inline-flex items-center gap-1 text-sm text-brand-light hover:text-white"
          >
            <ArrowLeft className="h-4 w-4" />
            Back to Orders
          </Link>
      ...
      {/* Status Timeline */}
      <div className="rounded-2xl border border-brand-medium/35 bg-brand-dark-alt p-6 sm:p-8">
        {isCancelled ? (
          ...
          <div className="relative">
            <div className="absolute left-0 top-[18px] hidden h-0.5 w-full bg-brand-medium/35 sm:block" />
            <div className="relative flex flex-col justify-between gap-8 sm:flex-row sm:gap-0">
              {statusSteps.map((step, idx) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`flex h-10 w-10 items-center justify-center rounded-full border-4 border-brand-dark-alt transition-colors ${
                        isCompleted
                          ? "bg-brand-orange text-white"
                          : "bg-brand-dark text-brand-light/50"
                      } ${isCurrent ? "ring-2 ring-brand-orange ring-offset-2 ring-offset-brand-dark-alt" : ""}`}
                    >
                      <Icon className="h-5 w-5" />
                    </div>
                    <span
                      className={`text-sm font-medium ${
                        isCompleted ? "text-white" : "text-brand-light/60"
                      }`}
                    >
                      {step.label}
                    </span>
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        ...
        <div className="space-y-4 lg:col-span-2">
          ...
          <div className="divide-y divide-brand-medium/35 overflow-hidden rounded-2xl border border-brand-medium/35 bg-brand-dark-alt">
            {order.order_items.map((item) => {
              const unit = parsePrice(item.product_price);
              return (
              <div key={item.id} className="flex gap-4 p-4 sm:p-6">
                <div className="h-20 w-20 flex-shrink-0 overflow-hidden rounded-xl bg-brand-dark">
        ...
        <div className="space-y-8">
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-2xl border border-brand-medium/35 bg-brand-dark-alt p-6">
              <p className="text-sm text-brand-light">Delivery Address</p>
              <div className="mt-2 text-brand-cream">
              ...
              {order.tracking_number && (
                <div className="mt-6 border-t border-brand-medium/35 pt-6">
                  <p className="text-sm text-brand-light">Tracking Number</p>
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-2xl border border-brand-medium/35 bg-brand-dark-alt p-6">
              <div className="space-y-3">
                ...
                <div className="mt-4 flex justify-between border-t border-brand-medium/35 pt-4 text-lg font-bold text-white">
                  <span>Total</span>
